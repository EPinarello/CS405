...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    { // COMPLETED: student need to change the next line from output[i] = source[i]
...
std::string read_file(const std::string& filename)
{
    ...
    // COMPLETED: implement loading the file into a string
    std::ifstream newFile(filename);
    std::getline(newFile, file_text, '\0');
...
{
    //  COMPLETED: implement file saving
    //  file format
    //  Line 1: student name
    ...
    saveFile << ctime(&time_stamp) << std::endl;
...
int main()
{
    std::cout << "EPinarello Module 5-2" << std::endl;
    std::cout << "Encryption Decryption Test!" << std::endl; //fixed typo in output
